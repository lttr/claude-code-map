...
chart, so the code and the UI call things *servitors*, *plates*, *heat* and *landmarks* rather
...
- **Invocation mark** — a skill/command's frontmatter restriction on who may invoke it. `user-only` (☞, from `disable-model-invocation: true`): slash-command only. `model-only` (✳, from `user-invocable: false`): no slash command. Unset means both may invoke.
- **Relation / route** — a declared reference from one skill or command to another, scraped from its body. Shown on the relations plate behind a toggle.

## Standing context

Second pillar next to servitors. Where a servitor is *invokable*, standing context is *always-on*:
...
- **Baseline** — the standing context loaded in *every* project: `~/.claude/CLAUDE.md`, `~/.claude/rules/*.md`, managed policy `/etc/claude-code/CLAUDE.md`. Shown once as the baseline strip, never repeated per card.
